...
def Spacer():
    return widget.Spacer(background=colors["dark"])


def sensors():
    return [
        widget.CPU(
            **base(bg="color3"), padding=10, margin_x=14, format="   {load_percent}%"
        ),
        widget.Memory(
            **base(bg="color3"), margin_x=14, padding=10, format="   {MemPercent}%"
        ),
    ]
